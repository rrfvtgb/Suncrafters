...
void Character::setBaseAnimation(int id, bool reset)
{
...
void Character::setTopAnimation(int id, bool reset){
    std::ofstream flux(std::string("report/Basecharacter.txt").c_str(), std::ios::app);

    if (this->mTopAnimID >= 0 && this->mTopAnimID < (int)this->mAnims.size()){
    ...
    if (id != ANIM_NONE){
        flux << "id = " << mTopAnimID << "; name : " << this->mAnims[id]->getAnimationName() << std::endl << std::endl;
