 ...
 *
 */
int LevelManager::getNumber(int seed, int max, int min)
{
    srand(seed);
    return ((double) rand()*(max-min)) / RAND_MAX + min;
}

Ogre::Vector2 LevelManager::getNumberFromCoord(int x, int y, int seed, int pass, int max)
{
    int gx = 0;
    int gy = 0;
    // generation of number
    srand(x/(pow(2,pass)*6));
    srand(seed+rand());
    if(pass != 0)
        gx = getNumber(rand(),max,-max);
    else
        gx = getNumber(rand(),max,5);
    srand(y/(pow(2,pass)*6));
    srand(seed+rand());
    if(pass != 0)
        gx = getNumber(rand(),max,-max);
    else
        gx = getNumber(rand(),max,5);
    return Ogre::Vector2(gx,gy);
}
/* if Block isNotAir
